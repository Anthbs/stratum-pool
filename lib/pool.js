...
var pool = module.exports = function pool(options, authorizeFn){
            ...
            */
            if (!isValidBlock) {
                ...
                SubmitBlock(blockHex, function(){
                    CheckBlockAccepted(shareData.blockHash, shareData, function(isAccepted, tx){
    ...
    function CheckVergeBlock(blockHash, shareData, callback) {
        _this.daemon.cmd('getblockbynumber',
            [shareData.height],
            function(results){
                var validResults = results.filter(function(result){
                    return result.response && (result.response.mined_hash === blockHash)
                });
                try {
                    fs.writeFile("verge_block_" + shareData.height + ".txt", blockHash + "\r\n" + validResults[0].response.hash, function() {});
                } catch(ex) {

                }
                if (validResults.length >= 1){
                    shareData.blockHash = validResults[0].response.hash;
    ...
    function CheckNevaBlock(blockHash, shareData, callback) {
        _this.daemon.cmd('getblockbynumber',
            [shareData.height],
            function(results){
                var validResults = results.filter(function(result){
                    return result.response && (result.response.proofhash === blockHash)
                });
                try {
                    fs.writeFile("neva_block_" + shareData.height + ".txt", blockHash + "\r\n" + validResults[0].response.hash, function() {});
                } catch(ex) {
    ...
    function CheckMyriadCoinBlock(blockHash, shareData, callback) {
        _this.daemon.cmd('getblockhash', [shareData.height], function(hashResult){
            console.log(hashResult);
            var hash = hashResult[0].response;
            _this.daemon.cmd('getblock', [hash], function(results){
                var validResults = results.filter(function(result){
                    return result.response && (result.response.pow_hash === blockHash)
                });
                try {
                    fs.writeFile("myriad_block_" + shareData.height + ".txt", blockHash + "\r\n" + validResults[0].response.hash, function() {});
                } catch(ex) {
    ...
    function CheckMyriadBlock(blockHash, shareData, coin, callback) {
        switch(coin.name.toLowerCase()) {
            case "verge":
                CheckVergeBlock(blockHash, shareData, callback);
                break;
            case "myriadcoin":
                CheckMyriadCoinBlock(blockHash, shareData, callback);
                break;
            case "neva":
                CheckNevaBlock(blockHash, shareData, callback);
                break;
            default: 
                callback(false);
                break;
    ...
    function CheckBlockAccepted(blockHash, shareData, callback){
        //setTimeout(function(){
            if(options.coin.algorithm == 'myr-gr') {
                CheckMyriadBlock(blockHash, shareData, options.coin, callback);
            } else {
                _this.daemon.cmd('getblock',
                    [blockHash],
                    function(results){
